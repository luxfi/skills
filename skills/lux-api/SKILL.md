...
name: lux-api
description: >
  Use this skill when the user wants to interact with blockchains via Lux Cloud APIs.
...
1. **API Key required.** If `LUX_API_KEY` is not set, tell the user to get one free at https://cloud.lux.network/dashboard, or switch to the `lux-gateway` skill for keyless access.
...
- `LUX_API_KEY` environment variable is set and non-empty
- If unset, suggest: `export LUX_API_KEY=<your-key>`

## Quick reference

| Item | Value |
|------|-------|
| Base URL | `https://api.cloud.lux.network/v1` |
| Auth header | `X-API-Key: ${LUX_API_KEY}` |
| Alt auth | URL path: `https://api.cloud.lux.network/v1/${LUX_API_KEY}/eth/mainnet` |
| WebSocket | `wss://ws.cloud.lux.network/v1/${LUX_API_KEY}` |
| Status | https://status.cloud.lux.network |
| Docs | https://cloud.lux.network/docs |
...
```bash
curl -X POST https://api.cloud.lux.network/v1/eth/mainnet \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ${LUX_API_KEY}" \
...
```bash
curl "https://api.cloud.lux.network/v1/data/tokens/balances?address=0x...&network=eth/mainnet" \
  -H "X-API-Key: ${LUX_API_KEY}"
```
...
```bash
curl "https://api.cloud.lux.network/v1/data/nfts?owner=0x...&network=eth/mainnet" \
  -H "X-API-Key: ${LUX_API_KEY}"
```
...
```bash
curl -X POST https://api.cloud.lux.network/v1/webhooks \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ${LUX_API_KEY}" \
  -d '{
...
- Dashboard: https://cloud.lux.network/dashboard
- Documentation: https://cloud.lux.network/docs
- API Reference: https://cloud.lux.network/docs/api
- Status: https://status.cloud.lux.network
- Support: https://cloud.lux.network/contact
